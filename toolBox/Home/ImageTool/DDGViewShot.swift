...
class DDGViewShot: BaseViewController {
    ...
    override func viewDidLoad() {
        ...
        // 获取导航栏下方的起始Y坐标
        let startY: CGFloat = statusBarAndNavigationBarHeight + 20
        
        blueView = UIView()
        blueView.backgroundColor = UIColor.blue
        blueView.center = CGPoint(x: self.view.center.x, y: startY + 100)
        blueView.frame.size = CGSize(width: 100, height: 100)
        self.view.addSubview(blueView)
        
        logoImageView = UIImageView(image: UIImage(named:"logo"))
        logoImageView.frame = CGRect(x: 50, y: startY, width: 100, height: 100)
        ...
        leftBtn.frame = CGRect(x: 20, y: height - bottomMargint - 40, width: (width - 60) / 2, height: 40)
        ...
        rightBtn.frame = CGRect(x: (width - 60) / 2 + 40, y: height - bottomMargint - 40, width: (width - 60) / 2, height: 40)
        self.view.addSubview(rightBtn)
        
        activity = UIActivityIndicatorView()
        activity.style = .whiteLarge
        activity.frame = CGRect(x:width / 2.0 - 15, y: startY + 20, width: 30, height: 30)
